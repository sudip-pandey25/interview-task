export const productData = [
  {
    id: 1,
    category: "Bedding",
    name: "OSP Home Furnishings Wicker Papasan Chair with 360-Degree Swivel, Brown Frame with Cream Cushion",
    description: [
      {
        des: "Create a laid back vibe in your home with our twist on the classic papasan chair design",
      },
      {
        des: "Nest into the generously large Dacron-filled cushion",
      },
      {
        des: "Enjoy the boho styling of our durable resin wicker wrapped over a metal frame",
      },
      {
        des: "Enjoy the boho styling of our durable resin wicker wrapped over a metal frame",
      },
      {
        des: "Enjoy the boho styling of our durable resin wicker wrapped over a metal frame",
      },
    ],
    image_Url: [
      {
        url: "/Rectangle 4.jpg",
      },
      {
        url: "https://cdn.shopify.com/s/files/1/0044/1208/0217/files/EMPIRE-SAMPLE-RDB-SRB-rattan-dark-brown_900x.jpg?v=1718878469",
      },
      {
        url: "https://s3-alpha-sig.figma.com/img/7123/e494/8a280d8510b14f00cbae2555d3589c8b?Expires=1738540800&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=RS0GOp0IeJ5HK3OVihfxYLzuDIJwbQ7BoGlq6swhfRV1JfJJOyMlbX-Hz0-SJ6WIzYIJJk2AB-T5SuZ8Mqob5yY8wq~7NJzdfJvQGzO2-9pLrb48Yx2tFBlacMaU4-x2-xjtDr0382hr2U-zNPj4QyrgbG6yYIB4N~QTL0hEuDU3krHhWKcZhjYJNR8ljFowXGM4476ADQv0V5aE8b1MLiCkZRd~4Ct7~TUNt010ZZcY8QgyHlloR5Li5ok84dMCsl2WEthAthxnc9Rk2bfWRM3gXPQvn3FzMF2Syfp~wc4kq-i4LPxEIegEj-Oz1QKj9YQiEq8mEaD6io4QWWKIfw__",
      },
      {
        url: "https://s3-alpha-sig.figma.com/img/7123/e494/8a280d8510b14f00cbae2555d3589c8b?Expires=1738540800&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4&Signature=RS0GOp0IeJ5HK3OVihfxYLzuDIJwbQ7BoGlq6swhfRV1JfJJOyMlbX-Hz0-SJ6WIzYIJJk2AB-T5SuZ8Mqob5yY8wq~7NJzdfJvQGzO2-9pLrb48Yx2tFBlacMaU4-x2-xjtDr0382hr2U-zNPj4QyrgbG6yYIB4N~QTL0hEuDU3krHhWKcZhjYJNR8ljFowXGM4476ADQv0V5aE8b1MLiCkZRd~4Ct7~TUNt010ZZcY8QgyHlloR5Li5ok84dMCsl2WEthAthxnc9Rk2bfWRM3gXPQvn3FzMF2Syfp~wc4kq-i4LPxEIegEj-Oz1QKj9YQiEq8mEaD6io4QWWKIfw__",
      },
    ],
    price: 1200,
    options: ["cream", "black", "orange", "blue"],
    discountAmount: "1,400",
    OriginalAmount: "1,540",
    stock: 3,
  },
];

export const FooterData = [
  {
    header: [
      {
        head: "CUSTOMER CARE",
      },
      {
        head: "ABOUT",
      },
      {
        head: "USER POLICY  ",
      },
      {
        head: "CONNECT WITH US ",
      },
      {
        head: "STAY CONNECTED",
      },
    ],
    description1: [
      {
        des: "Account",
      },
      {
        des: "My Orders",
      },
      {
        des: "Wishlist",
      },
      {
        des: "FAQs",
      },
      {
        des: "Support",
      },
    ],
    description2: [
      {
        des: "Company",
      },
      {
        des: "Info",
      },
    ],
    description3: [
      {
        des: "Privacy Policy",
      },
      {
        des: "Terms and Conditions",
      },
      {
        des: "Data Policy",
      },
      {
        des: "Coookies Policy",
      },
      {
        des: "Refund Policy",
      },
      {
        des: "Return Policy",
      },
    ],
    description4: [
      {
        des: "Account",
      },
    ],
    description5: [
      {
        des: "Baneshwor, Kathmandu, Nepal",
      },
      {
        des: "+977-9100000000",
      },
      {
        des: "test@gmail.com",
      },
    ],
  },
];

export const FooterTableData = [
  {
    row: [{ head: "Item Weight", Details: "29 pounds" }],
  },
  {
    row: [{ head: "Material", Details: "Aluminum" }],
  },
  {
    row: [{ head: "Dimensions", Details: "20x30x10 cm" }],
  },
];
